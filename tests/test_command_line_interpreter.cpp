...
TEST_CASE("CommandLineInterpreter pwd runs without error") {
  ...
  bool no_pwd_error =
      err_str.empty() || err_str.find("pwd") == std::string::npos;
  CHECK(no_pwd_error);
  std::string out_str = out.str();
  bool has_path_sep = out_str.find('/') != std::string::npos ||
                      out_str.find('\\') != std::string::npos;
  CHECK(has_path_sep);
}
...
TEST_CASE("CommandLineInterpreter empty command in middle of pipeline reports "
          "error") {
  CommandLineInterpreter cli;
