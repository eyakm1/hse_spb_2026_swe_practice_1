...
/** Resolves executable using PATH on Windows; returns full path or original
 * name. */
std::string resolve_executable_win32(const Environment &env,
...
std::string resolve_executable(const Environment &env,
                               const std::string &name) {
  if (name.find('/') != std::string::npos)
    return name;
  std::string path_var = env.get("PATH");
...
int ExternalCommand::execute(const std::vector<std::string> &args,
  ...
  BOOL ok = CreateProcessA(
      program_path.c_str(), cmdline_buf.data(), nullptr, nullptr, TRUE,
      CREATE_NO_WINDOW,
  ...
  std::thread writer([&in, hStdinW]() {
    ...
    while (in.read(buf.data(), buf.size()) || in.gcount() > 0) {
      DWORD written = 0;
      WriteFile(hStdinW, buf.data(), static_cast<DWORD>(in.gcount()), &written,
                nullptr);
    }
    CloseHandle(hStdinW);
  });
  ...
  auto read_handle = [](HANDLE h, std::ostream &o) {
    std::array<char, 4096> buf;
    DWORD read_len = 0;
    while (ReadFile(h, buf.data(), static_cast<DWORD>(buf.size()), &read_len,
                    nullptr) &&
           read_len > 0)
