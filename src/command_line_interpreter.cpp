...
/** Strip leading VAR=value from first command and apply to env. Mutates pipeline. */
void apply_assignments(Pipeline& pipeline, Environment& env) {
    ...
    for (std::size_t i = 0; i < first.args.size(); ++i) {
        Substitute sub = (i < first.substitute_arg.size()) ? first.substitute_arg[i] : Substitute::Yes;
...
void drop_empty_leading_commands(Pipeline& pipeline) {
    while (!pipeline.empty() && pipeline.front().name.empty() && pipeline.front().args.empty()) {
