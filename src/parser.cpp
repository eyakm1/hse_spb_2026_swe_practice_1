...
/** Tokenize one pipeline segment; fill tokens and substitute flags (Single=No, Double/Unquoted=Yes). */
void tokenize_segment(const std::string& segment,
    ...
    while (i < n) {
        ...
        while (i < n && !is_space(segment[i]) && segment[i] != '\'' && segment[i] != '"') {
            token += segment[i];
            ++i;
        }
        tokens.push_back(std::move(token));
...
std::optional<Pipeline> Parser::parse(const std::string& line) const {
    ...
    for (const auto& seg : segments) {
        ...
        node.substitute_name = substitute_flags.empty() ? Substitute::Yes : substitute_flags[0];
        for (std::size_t j = 1; j < tokens.size(); ++j) {
            node.args.push_back(std::move(tokens[j]));
            node.substitute_arg.push_back(j < substitute_flags.size() ? substitute_flags[j] : Substitute::Yes);
        }
